...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        stump_error = (np.sum(sample_weights*Wrong))/(np.sum(sample_weights))
        return stump_error
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        if(Error == 0):
            a = np.exp(alpha * (np.not_equal(y, y_pred)).astype(int))
            return (sample_weights)*a

        neg_error = 1-Error
        for i in range(len(sample_weights)):
            if y[i] == y_pred[i]:
                sample_weights[i] /= 2*neg_error
            else:
                sample_weights[i] /= 2*Error
    ...
    def predict(self, X):
        ...
        a = [self.stumps[stump].predict(X) for stump in range(self.n_stumps)]
        predictionn = np.array(a)
        return np.sign(predictionn[0])
        # TODO
        #pass
