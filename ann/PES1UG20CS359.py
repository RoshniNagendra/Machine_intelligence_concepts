...
class Tensor:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, self.__class__):
            ...
            output_tensor = Tensor(self.arr + other.arr)
            output_tensor.set_history('add', self, other)
    ...
    def __matmul__(self, other):
        ...
        output_tensor = Tensor(self.arr @ other.arr)
        output_tensor.set_history('matmul', self, other)
    ...
    def grad_add(self, gradients=None):
        ...
        """
        # TODO
        o1,o2= self.history[1],self.history[2]
        o1.grad,o2.grad = np.zeros_like(o1.arr),np.zeros_like(o2.arr)
        if o1.requires_grad: 
            o1.grad += np.ones_like(o1.arr)
        if o2.requires_grad: 
            o2.grad += np.ones_like(o2.arr)
        if gradients is None:
            return (o1.grad,o2.grad)
        if o1.requires_grad: 
            o1.grad = np.multiply(np.ones_like(o1.arr), gradients)
        if o2.requires_grad: 
            o2.grad = np.multiply(np.ones_like(o2.arr), gradients)
        return (o1.grad, o2.grad)
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
            try:
                o1.grad += np.matmul(np.ones_like(o1.arr), o2.arr.transpose())
            except:
                pass
            try:
                o2.grad += (np.matmul(np.ones_like(o2.arr), o1.arr)).transpose()
            except:
                pass
        else:
            try:
                o1.grad += np.multiply(np.matmul(np.ones_like(o1.arr), o2.arr.transpose()), gradients)
            except:
                pass
            try:
                o2.grad += np.multiply(np.matmul(np.ones_like(o2.arr), o1.arr).transpose(), gradients)
            except:
                pass
        return (o1.grad, o2.grad)
